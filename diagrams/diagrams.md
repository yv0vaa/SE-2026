...
## Практика 5: Моделирование требований
...
## Практика 6: Моделирование структуры
...
## Практика 7: Моделирование поведения
...
### 8. Диаграмма состояний дефекта (State Machine Diagram)

Диаграмма описывает жизненный цикл дефекта от момента обнаружения до завершения ремонта.

**Состояния:**
- **Обнаружен**: Автомобиль снят с конвейера и направлен в ремонтную зону
- **Диагностика**: Выполняется первоначальная диагностика дефекта
- **Зарегистрирован**: Дефект зарегистрирован в системе (место дефекта, причина, номер автомобиля, дата поступления, ремонтник-диагност)
- **Назначен**: Бригадир назначил ремонт конкретному рабочему
- **ВРемонте**: Ремонт выполняется (зафиксированы время начала, ремонтное место, ремонтник)
- **Отремонтирован**: Ремонт завершён (зафиксированы время конца, рабочий, выполнивший ремонт)
- **ВозвращёнНаКонвейер**: Автомобиль возвращён на конвейер или в зону тестирования

```mermaid
stateDiagram-v2
    [*] --> Обнаружен: Автомобиль поступил в ремонтную зону
    
    Обнаружен --> Диагностика: Начало диагностики
    Диагностика --> Зарегистрирован: Регистрация дефекта в системе
    
    Зарегистрирован --> Назначен: Бригадир назначил ремонт рабочему
    Назначен --> ВРемонте: Ремонтник зафиксировал начало ремонта
    
    ВРемонте --> Отремонтирован: Ремонтник зафиксировал конец ремонта
    Отремонтирован --> ВозвращёнНаКонвейер: Автомобиль возвращён на конвейер
    
    ВозвращёнНаКонвейер --> [*]: Процесс завершён
```

### 9. Диаграмма последовательностей генерации отчётов (Sequence Diagram)

```mermaid
sequenceDiagram
    participant ОКК как Отдел контроля качества
    participant НЦ как Начальник цеха
    participant Б как Бригадир
    participant Бух как Бухгалтерия
    participant С как Система
    participant БД как База данных
    participant ОтчётГен как Генератор отчётов
    
    Note over С,БД: Конец смены - автоматическая генерация отчётов
    
    С->>ОтчётГен: Запустить генерацию отчётов
    
    Note over ОтчётГен,БД: Генерация отчёта для Отдела контроля качества
    ОтчётГен->>БД: Получить данные по ремонтам за смену
    БД-->>ОтчётГен: Данные о ремонтах
    ОтчётГен->>БД: Получить статистику по местам дефекта
    БД-->>ОтчётГен: Статистика
    ОтчётГен->>БД: Вычислить среднее время ремонта
    БД-->>ОтчётГен: Среднее время
    ОтчётГен->>С: Сформировать сводный отчёт
    С->>ОКК: Отправить сводный отчёт<br/>(количество, статистика, среднее время, детали)
    
    Note over ОтчётГен,БД: Генерация отчёта для Начальника цеха
    ОтчётГен->>БД: Получить общее количество ремонтов
    БД-->>ОтчётГен: Количество
    ОтчётГен->>БД: Получить суммарное время ремонтов
    БД-->>ОтчётГен: Суммарное время
    ОтчётГен->>С: Сформировать отчёт
    С->>НЦ: Отправить отчёт<br/>(общее количество, суммарное время)
    
    Note over ОтчётГен,БД: Генерация отчёта для Бригадира
    loop Для каждой бригады
        ОтчётГен->>БД: Получить список ремонтников бригады
        БД-->>ОтчётГен: Список ремонтников
        loop Для каждого ремонтника
            ОтчётГен->>БД: Получить количество ремонтов
            БД-->>ОтчётГен: Количество
            ОтчётГен->>БД: Получить время на каждый ремонт
            БД-->>ОтчётГен: Детали ремонтов
            ОтчётГен->>БД: Вычислить суммарное время
            БД-->>ОтчётГен: Суммарное время
        end
        ОтчётГен->>С: Сформировать отчёт по бригаде
        С->>Б: Отправить отчёт<br/>(по каждому ремонтнику: количество, время)
    end
    
    Note over ОтчётГен,БД: Генерация месячного отчёта для Бухгалтерии
    ОтчётГен->>БД: Получить всех ремонтников
    БД-->>ОтчётГен: Список ремонтников
    loop Для каждого ремонтника
        ОтчётГен->>БД: Подсчитать отработанные смены за месяц
        БД-->>ОтчётГен: Количество смен
    end
    ОтчётГен->>С: Сформировать месячный отчёт
    С->>Бух: Отправить отчёт<br/>(по каждому ремонтнику: количество смен)
```
